...
async function laadAanvragen() {
  try {
    ...
    console.error("Aanvragen laden fout:", err);
    document.getElementById("aanvragenBody").innerHTML =
...
function updateTabCounts() {
  ...
  document.getElementById("tabWijzigCount").textContent =
    alleStages.filter(s => s.status === "aanpassingen_vereist").length;
  document.getElementById("tabWachtCount").textContent =
    alleStages.filter(s => s.status === "ingediend").length;
}
...
function renderTabel() {
  ...
  const tbody = document.getElementById("aanvragenBody");
  if (gefilterd.length === 0) {
    tbody.innerHTML = `<tr><td colspan="6" class="table-empty">Geen aanvragen gevonden</td></tr>`;
    return;
  }
